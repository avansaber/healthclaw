...
|--------|---------------|----------------|
| `dental-add-tooth-chart-entry` | `--patient-id --company-id --tooth-number --condition --noted-date` | `--tooth-system --surface --condition-detail --noted-by-id --notes` |
| `dental-update-tooth-chart-entry` | `--tooth-chart-id` | `--condition --condition-detail --surface --status --notes` |
| `dental-get-tooth-chart` | `--patient-id` | |

### Dental Procedures (2 actions)
| Action | Required Flags | Optional Flags |
|--------|---------------|----------------|
| `dental-add-dental-procedure` | `--encounter-id --patient-id --company-id --provider-id --cdt-code --procedure-date` | `--cdt-description --tooth-number --surface --quadrant --fee --notes` |
| `dental-list-dental-procedures` | | `--encounter-id --patient-id --cdt-code --status --search --limit --offset` |

### Treatment Plans (3 actions)
| Action | Required Flags | Optional Flags |
|--------|---------------|----------------|
| `dental-add-treatment-plan` | `--patient-id --company-id --provider-id --plan-name --plan-date` | `--phases --estimated-total --insurance-estimate --patient-estimate --notes` |
| `dental-update-treatment-plan` | `--treatment-plan-id` | `--plan-name --status --phases --estimated-total --insurance-estimate --patient-estimate --notes` |
| `dental-list-treatment-plans` | | `--patient-id --status --search --limit --offset` |

### Periodontal Charting (4 actions)
| Action | Required Flags | Optional Flags |
|--------|---------------|----------------|
| `dental-add-perio-exam` | `--patient-id --company-id --provider-id --exam-date` | `--measurements --bleeding-sites --furcation-data --mobility-data --recession-data --plaque-score --notes` |
| `dental-get-perio-exam` | `--perio-exam-id` | |
| `dental-list-perio-exams` | `--patient-id` | `--limit --offset` |
| `dental-compare-perio-exams` | `--exam-id-1 --exam-id-2` | |
