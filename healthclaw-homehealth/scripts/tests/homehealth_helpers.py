"""Shared helper functions for HealthClaw Home Health unit tests.

Provides:
  - DB bootstrap via init_schema.init_db() + create_healthclaw_tables() + init_homehealth_schema()
...
ROOT_DIR = os.path.dirname(MODULE_DIR)                     # healthclaw-homehealth/
...
# Home Health init
HOMEHEALTH_INIT_PATH = os.path.join(ROOT_DIR, "init_db.py")
...
def load_db_query():
    """Load healthclaw-homehealth's db_query.py explicitly to avoid sys.path collisions."""
    db_query_path = os.path.join(MODULE_DIR, "db_query.py")
    spec = importlib.util.spec_from_file_location("db_query_homehealth", db_query_path)
...
def init_all_tables(db_path: str):
    """Create all foundation + core healthclaw + home health tables."""
    # 1. Foundation tables
    ...
    # 3. Home Health tables (4 tables)
    spec3 = importlib.util.spec_from_file_location("homehealth_init", HOMEHEALTH_INIT_PATH)
    m3 = importlib.util.module_from_spec(spec3)
    spec3.loader.exec_module(m3)
    m3.init_homehealth_schema(db_path)
...
def seed_company(conn, name="Test Home Health Co", abbr="THH") -> str:
    cid = _uuid()
    conn.execute(
...
def seed_employee(conn, company_id: str, name="Nurse Home") -> str:
    eid = _uuid()
    parts = name.split(" ", 1)
    first = parts[0]
...
def seed_patient(conn, company_id: str, first="Martha", last="Jones") -> str:
    pid = _uuid()
    ...
    conn.execute(
        """INSERT INTO healthclaw_patient
           ...
           VALUES (?, ?, ?, ?, ?, '1945-08-10', 'female', 'active', ?, ?, ?)""",
...
def build_env(conn) -> dict:
    """Create a full home health test environment."""
    cid = seed_company(conn)
    clinician = seed_employee(conn, cid, "Nurse Practitioner")
    aide = seed_employee(conn, cid, "Home Health Aide")
    physician = seed_employee(conn, cid, "Dr. Certifying Physician")
    seed_naming_series(conn, cid)
    patient = seed_patient(conn, cid, "Martha", "Jones")
