...
|--------|---------------|----------------|
| `homehealth-add-home-visit` | `--patient-id --company-id --clinician-id --visit-date --visit-type` | `--start-time --end-time --travel-time-minutes --mileage --visit-status --notes` |
| `homehealth-update-home-visit` | `--home-visit-id` | `--visit-date --visit-type --start-time --end-time --travel-time-minutes --mileage --visit-status --notes` |
| `homehealth-list-home-visits` | | `--patient-id --clinician-id --visit-type --visit-status --limit --offset` |

### Care Plans (4 actions)
| Action | Required Flags | Optional Flags |
|--------|---------------|----------------|
| `homehealth-add-care-plan` | `--patient-id --company-id --start-of-care --certification-period-start --certification-period-end` | `--certifying-physician-id --frequency --goals --notes` |
| `homehealth-update-care-plan` | `--care-plan-id` | `--certification-period-start --certification-period-end --frequency --goals --plan-status --notes` |
| `homehealth-get-care-plan` | `--care-plan-id` | |
| `homehealth-list-care-plans` | | `--patient-id --plan-status --limit --offset` |

### OASIS Assessments (2 actions)
| Action | Required Flags | Optional Flags |
|--------|---------------|----------------|
| `homehealth-add-oasis-assessment` | `--patient-id --company-id --clinician-id --assessment-type --assessment-date` | `--m-items --notes` |
| `homehealth-list-oasis-assessments` | | `--patient-id --assessment-type --limit --offset` |

### Aide Assignments (3 actions)
| Action | Required Flags | Optional Flags |
|--------|---------------|----------------|
| `homehealth-add-aide-assignment` | `--patient-id --company-id --aide-id --assignment-start` | `--assignment-end --days-of-week --visit-time --tasks --supervisor-id --supervision-due-date --notes` |
| `homehealth-update-aide-assignment` | `--aide-assignment-id` | `--assignment-end --days-of-week --visit-time --tasks --supervisor-id --supervision-due-date --status --notes` |
| `homehealth-list-aide-assignments` | | `--patient-id --aide-id --status --limit --offset` |
