"""Shared helper functions for HealthClaw Mental Health unit tests.

Provides:
  - DB bootstrap via init_schema.init_db() + create_healthclaw_tables() + init_mental_schema()
  - call_action() / ns() / is_error() / is_ok()
  - Seed functions for company, employee, naming series, patient, encounter
...
ROOT_DIR = os.path.dirname(MODULE_DIR)                     # healthclaw-mental/
...
# Mental health init
MENTAL_INIT_PATH = os.path.join(ROOT_DIR, "init_db.py")
...
def load_db_query():
    """Load healthclaw-mental's db_query.py explicitly to avoid sys.path collisions."""
    db_query_path = os.path.join(MODULE_DIR, "db_query.py")
    spec = importlib.util.spec_from_file_location("db_query_mental", db_query_path)
...
def init_all_tables(db_path: str):
    """Create all foundation + core healthclaw + mental health tables."""
    # 1. Foundation tables
    ...
    # 3. Mental health tables (4 tables)
    spec3 = importlib.util.spec_from_file_location("mental_init", MENTAL_INIT_PATH)
    m3 = importlib.util.module_from_spec(spec3)
    spec3.loader.exec_module(m3)
    m3.init_mental_schema(db_path)
...
def seed_company(conn, name="Test Mental Health Co", abbr="TMH") -> str:
    cid = _uuid()
    conn.execute(
...
def seed_employee(conn, company_id: str, name="Dr. Therapist") -> str:
    eid = _uuid()
    parts = name.split(" ", 1)
    first = parts[0]
...
def seed_patient(conn, company_id: str, first="Jane", last="Smith") -> str:
    ...
    conn.execute(
        """INSERT INTO healthclaw_patient
           ...
           VALUES (?, ?, ?, ?, ?, '1990-05-15', 'female', 'active', ?, ?, ?)""",
...
def seed_encounter(conn, company_id: str, patient_id: str, provider_id: str) -> str:
    ...
    conn.execute(
        """INSERT INTO healthclaw_encounter
           ...
           VALUES (?, ?, ?, ?, '2026-03-15', 'outpatient', 'open', ?, ?, ?)""",
...
def build_env(conn) -> dict:
    """Create a full mental health test environment."""
    cid = seed_company(conn)
    provider = seed_employee(conn, cid, "Dr. Therapist")
