...
|--------|---------------|----------------|
| `mentalhealth-add-therapy-session` | `--encounter-id --patient-id --company-id --provider-id --session-type` | `--modality --duration-minutes --session-number --notes --status` |
| `mentalhealth-update-therapy-session` | `--therapy-session-id` | `--session-type --modality --duration-minutes --notes --status` |
| `mentalhealth-list-therapy-sessions` | | `--patient-id --provider-id --status --search --limit --offset` |

### Assessments (4 actions)
| Action | Required Flags | Optional Flags |
|--------|---------------|----------------|
| `mentalhealth-add-assessment` | `--patient-id --company-id --instrument --administered-date` | `--administered-by-id --responses --score --notes` |
| `mentalhealth-get-assessment` | `--assessment-id` | |
| `mentalhealth-list-assessments` | | `--patient-id --instrument --limit --offset` |
| `mentalhealth-compare-assessments` | `--assessment-id-1 --assessment-id-2` | |

### Treatment Goals (3 actions)
| Action | Required Flags | Optional Flags |
|--------|---------------|----------------|
| `mentalhealth-add-treatment-goal` | `--patient-id --company-id --goal-description` | `--provider-id --target-date --baseline-measure --current-measure --notes` |
| `mentalhealth-update-treatment-goal` | `--treatment-goal-id` | `--goal-description --target-date --current-measure --goal-status --notes` |
| `mentalhealth-list-treatment-goals` | | `--patient-id --goal-status --limit --offset` |

### Group Sessions (4 actions)
| Action | Required Flags | Optional Flags |
|--------|---------------|----------------|
| `mentalhealth-add-group-session` | `--company-id --provider-id --session-date --group-name` | `--group-type --topic --max-participants --participant-ids --duration-minutes --notes --status` |
| `mentalhealth-update-group-session` | `--group-session-id` | `--group-name --group-type --topic --participant-ids --duration-minutes --notes --status` |
| `mentalhealth-list-group-sessions` | | `--provider-id --status --search --limit --offset` |
| `mentalhealth-get-group-session` | `--group-session-id` | |
