"""Shared helper functions for HealthClaw Vet unit tests.

Provides:
  - DB bootstrap via init_schema.init_db() + create_healthclaw_tables() + init_vet_schema()
  - call_action() / ns() / is_error() / is_ok()
  - Seed functions for company, employee, naming series, patient
...
ROOT_DIR = os.path.dirname(MODULE_DIR)                     # healthclaw-vet/
...
# Vet init
VET_INIT_PATH = os.path.join(ROOT_DIR, "init_db.py")
...
def load_db_query():
    """Load healthclaw-vet's db_query.py explicitly to avoid sys.path collisions."""
    db_query_path = os.path.join(MODULE_DIR, "db_query.py")
    spec = importlib.util.spec_from_file_location("db_query_vet", db_query_path)
...
def init_all_tables(db_path: str):
    """Create all foundation + core healthclaw + vet tables."""
    # 1. Foundation tables
    ...
    # 3. Vet tables (4 tables)
    spec3 = importlib.util.spec_from_file_location("vet_init", VET_INIT_PATH)
    m3 = importlib.util.module_from_spec(spec3)
    spec3.loader.exec_module(m3)
    m3.init_vet_schema(db_path)
...
def seed_company(conn, name="Test Vet Co", abbr="TVC") -> str:
    cid = _uuid()
    conn.execute(
...
def seed_employee(conn, company_id: str, name="Dr. Vet Provider") -> str:
    eid = _uuid()
    parts = name.split(" ", 1)
    first = parts[0]
...
def seed_patient(conn, company_id: str, first="Buddy", last="Dog") -> str:
    """Insert a healthclaw_patient (the animal's medical record)."""
    pid = _uuid()
...
def seed_animal_patient(conn, company_id: str, patient_id: str,
                        species="canine", breed="Labrador", weight_kg="30.00") -> str:
    """Insert a healthclaw_animal_patient linked to a patient record."""
    aid = _uuid()
    now = "2026-01-01T00:00:00Z"
    conn.execute(
        """INSERT INTO healthclaw_animal_patient
           (id, company_id, patient_id, species, breed, color, weight_kg,
            microchip_id, spay_neuter_status, reproductive_status, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, 'golden', ?, NULL, 'neutered', NULL, ?, ?)""",
        (aid, company_id, patient_id, species, breed, weight_kg, now, now)
    )
    conn.commit()
    return aid
...
def build_env(conn) -> dict:
    """Create a full vet test environment."""
    cid = seed_company(conn)
    provider = seed_employee(conn, cid, "Dr. Vet Provider")
    seed_naming_series(conn, cid)
    patient = seed_patient(conn, cid, "Buddy", "Dog")
    animal = seed_animal_patient(conn, cid, patient)

    return {
        "company_id": cid,
        "provider_id": provider,
        "patient_id": patient,
        "animal_patient_id": animal,
    }
