...
|--------|---------------|----------------|
| `vet-add-animal-patient` | `--company-id --patient-id --species` | `--breed --color --weight-kg --microchip-id --spay-neuter-status --reproductive-status` |
| `vet-update-animal-patient` | `--animal-patient-id` | `--species --breed --color --weight-kg --microchip-id --spay-neuter-status --reproductive-status` |
| `vet-get-animal-patient` | `--animal-patient-id` | |
| `vet-list-animal-patients` | | `--company-id --species --search --limit --offset` |

### Boarding (3 actions)
| Action | Required Flags | Optional Flags |
|--------|---------------|----------------|
| `vet-add-boarding` | `--company-id --animal-patient-id --check-in-date` | `--check-out-date --kennel-number --feeding-instructions --medication-schedule --special-needs --daily-rate --notes` |
| `vet-update-boarding` | `--boarding-id` | `--check-out-date --kennel-number --feeding-instructions --medication-schedule --special-needs --daily-rate --status --notes` |
| `vet-list-boardings` | | `--animal-patient-id --status --limit --offset` |

### Weight-Based Dosing (2 actions)
| Action | Required Flags | Optional Flags |
|--------|---------------|----------------|
| `vet-calculate-dose` | `--animal-patient-id --company-id --medication-name --dose-per-kg` | `--weight-kg --weight-date --dose-unit --route --frequency --notes` |
| `vet-list-dosing-history` | | `--animal-patient-id --medication-name --limit --offset` |

### Owner Links (3 actions)
| Action | Required Flags | Optional Flags |
|--------|---------------|----------------|
| `vet-add-owner-link` | `--company-id --animal-patient-id --owner-name` | `--owner-phone --owner-email --relationship --is-primary --financial-responsibility --notes` |
| `vet-update-owner-link` | `--owner-link-id` | `--owner-name --owner-phone --owner-email --relationship --is-primary --financial-responsibility --notes` |
| `vet-list-owner-links` | | `--animal-patient-id --limit --offset` |
