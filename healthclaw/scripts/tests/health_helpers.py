"""Shared helper functions for HealthClaw core unit tests.

Provides:
  - DB bootstrap via init_schema.init_db() + create_healthclaw_tables()
  - call_action() / ns() / is_error() / is_ok()
  - Seed functions for company, employee, customer, naming series
...
ROOT_DIR = os.path.dirname(MODULE_DIR)                     # healthclaw/
PARENT_DIR = os.path.dirname(ROOT_DIR)                     # src/healthclaw/
...
def load_db_query():
    """Load healthclaw's db_query.py explicitly to avoid sys.path collisions."""
    db_query_path = os.path.join(MODULE_DIR, "db_query.py")
    spec = importlib.util.spec_from_file_location("db_query_health", db_query_path)
...
def init_all_tables(db_path: str):
    """Create all foundation + healthclaw tables."""
    ...
    # HealthClaw vertical tables (40 tables)
    spec2 = importlib.util.spec_from_file_location("health_init", VERTICAL_INIT_PATH)
...
def seed_company(conn, name="Test Health Co", abbr="THC") -> str:
...
def seed_employee(conn, company_id: str, name="Dr. Test Provider") -> str:
    """Insert an employee (provider) and return its ID."""
    eid = _uuid()
    parts = name.split(" ", 1)
    first = parts[0]
...
def seed_naming_series(conn, company_id: str):
    """Seed naming series for HealthClaw entity types."""
    series = [
...
def seed_patient(conn, company_id: str, first="John", last="Doe") -> str:
    """Insert a healthclaw_patient and return its ID."""
    pid = _uuid()
...
def seed_encounter(conn, company_id: str, patient_id: str, provider_id: str) -> str:
    """Insert a healthclaw_encounter and return its ID."""
    eid = _uuid()
    now = "2026-01-01T00:00:00Z"
    conn.execute(
...
def seed_lab_order(conn, company_id: str, patient_id: str,
                   encounter_id: str, provider_id: str) -> str:
    """Insert a healthclaw_lab_order and return its ID."""
    oid = _uuid()
    now = "2026-01-01T00:00:00Z"
    conn.execute(
        """INSERT INTO healthclaw_lab_order
           (id, naming_series, encounter_id, patient_id, ordering_provider_id,
            order_date, priority, order_status, company_id, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, '2026-01-15', 'routine', 'ordered', ?, ?, ?)""",
        (oid, f"LAB-SEED-{oid[:6]}", encounter_id, patient_id, provider_id,
         company_id, now, now)
    )
    conn.commit()
    return oid
...
def build_env(conn) -> dict:
    """Create a full health test environment.

    Returns dict with all IDs needed for health domain tests.
    """
    cid = seed_company(conn)
    provider = seed_employee(conn, cid, "Dr. Test Provider")
    provider2 = seed_employee(conn, cid, "Dr. Second Provider")
    cust = seed_customer(conn, cid, "Health Customer")
    seed_naming_series(conn, cid)
    patient = seed_patient(conn, cid, "Jane", "Smith")
    encounter = seed_encounter(conn, cid, patient, provider)
    lab_order = seed_lab_order(conn, cid, patient, encounter, provider)
